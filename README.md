...
Relationship reasoning: `POST /documents/{id}/relationships` discovers cross-document candidate pairs (MiniLM embeddings for ranking only, lexical fallback when unavailable, top-k bounded — never all-pairs) and classifies them deterministically into `CORROBORATES` / `CONTRADICTS` / `CONTEXTUAL_DIFFERENCE` / `RELATED` (`UNRELATED` is counted, not stored) with explanations, confidence, and `RESOLVED` / `NEEDS_REVIEW` status. Gemini is consulted only for genuinely ambiguous pairs and never performs arithmetic; without a key those pairs persist as low-confidence review items. `GET /documents/{id}/relationships` lists them (type/min-confidence filters); `GET /relationships/{id}` returns both facts with their evidence. Embeddings run CPU-only (torch `2.14.0+cpu`, no CUDA stack); the Docker image pre-warms MiniLM weights at build time. Tune with `MATCHING_TOP_K`, `MATCHING_SIMILARITY_FLOOR`, and `MATCHING_NUMERIC_TOLERANCE`.
